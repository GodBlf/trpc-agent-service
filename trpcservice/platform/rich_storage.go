...
func (s *SQLStore) PutArtifact(ctx context.Context, item Artifact) (Artifact, error) {
	...
	_, err := s.db.ExecContext(ctx, s.q(`INSERT INTO artifacts(tenant_id,artifact_id,session_id,name,content_reference,request_id,trace_id,status,created_at) VALUES(?,?,?,?,?,?,?,?,?) ON CONFLICT(tenant_id,artifact_id) DO NOTHING`), item.TenantID, item.ID, item.SessionID, item.Name, item.ContentRef, item.RequestID, item.TraceID, item.Status, item.CreatedAt.Format(time.RFC3339Nano))
	return item, err
}

func (s *SQLStore) ListArtifacts(ctx context.Context, tenantID, sessionID string) ([]Artifact, error) {
	rows, err := s.db.QueryContext(ctx, s.q(`SELECT artifact_id,session_id,name,content_reference,request_id,trace_id,status,created_at FROM artifacts WHERE tenant_id=? AND (?='' OR session_id=?) ORDER BY created_at`), tenantID, sessionID, sessionID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ID, &item.SessionID, &item.Name, &item.ContentRef, &item.RequestID, &item.TraceID, &item.Status, &created); err != nil {
			return nil, err
		}

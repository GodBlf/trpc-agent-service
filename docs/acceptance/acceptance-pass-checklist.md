...
本次“通过”只表示对应单项有 README、代码或自动化证据支撑；未列入的要求不视为通过，也不代表主办方尚未公布的规则已被确认。真实模型与真实外部 IM 凭据 smoke 未纳入自动化通过项；S3、Qdrant/Milvus 和 Kubernetes 仅作为有落地边界的设计交付，不作为已完成的生产集成声明。

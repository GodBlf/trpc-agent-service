...
| --- | --- | --- | --- |
| PostgreSQL | 控制面、Session Event、Lease、权威 Memory、Artifact/Knowledge 元数据、共享治理运行态的未来目标 | 事务强一致；组合主键、唯一幂等键、row lock 和 fencing 校验 | Control Plane、Session/Memory、元数据、Lease 已实现；Compose 默认 |
| SQLite | 单节点控制面、开发环境 Session/Memory 和本地验证数据 | 单文件事务；适合单进程，不承担多 Gateway 协调 | 已实现开发适配器 |
| Redis | 热 Session/Memory、短期去重、限流和预算计数 | 单 key 原子；跨 key 事务和持久恢复较弱，持久性取决于配置 | Session/Memory 参考适配器已实现 |
| InMemory | 单元测试、fixture、无外部依赖的本地演示 | 仅进程内，重启丢失，不提供跨节点一致性 | 已实现，仅测试/开发 |
| Qdrant/Milvus | Knowledge 与 Memory 的语义向量索引 | 最终一致的派生索引；按 Tenant collection/partition 和 metadata 双重隔离 | 仅设计适配边界 |
| S3 兼容对象存储 | Artifact 大对象、附件、知识源文件 | 对象写后与 SQL metadata 两阶段发布；checksum、版本和孤儿回收 | 仅设计适配边界 |
...
已支持的运行时选择是 `inmemory`、`redis`、`sqlite` 和 `postgres`。创建失败或后端不可达时注册 unavailable store，使 API 返回明确的 `storage_unavailable`，而不是静默回退到默认后端。多 Gateway 下 Backend Selection 从共享 PostgreSQL Control Plane 刷新 revision，不能依赖各进程的旧本地配置。
...
S3 适合 Artifact 二进制内容、IM 附件和 Knowledge 原始文件。对象 key 至少包含 Tenant 和不可猜测 ID，不直接使用用户文件名作为授权边界。上传先写临时 key，校验大小与 checksum，再由 SQL 事务发布 metadata；读取只允许访问 published 且 Tenant 匹配的引用。

需要启用对象版本、生命周期规则、临时对象扫描和孤儿回收。内容成功但 metadata 失败时清理临时对象；metadata 已发布但对象不可读时标记 `recovery_required` 并拒绝虚假成功。备份和恢复以对象版本配合 PostgreSQL PITR 的一致时间点为准。
